...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getQueueToken('registration'), useValue: mockRegistrationQueue },
      ],
    }).compile();
  ...
  describe('signIn', () => {
    it('should return an access token for valid credentials', async () => {
      ...
      jest.spyOn(authService as any, 'comparePasswordInWorker').mockResolvedValue(true);
    ...
    it('should throw an ApiException for invalid credentials', async () => {
      ...
      await expect(authService.signIn('test@example.com', 'wrongpassword'))
        .rejects.toThrow(new ApiException('Invalid credentials', ErrorCode.INVALID_CREDENTIALS, 401));
    });
  });
  ...
  describe('register', () => {
    it('should add a registration job to the queue', async () => {
      ...
      expect(mockRegistrationQueue.add).toHaveBeenCalledWith('process-registration', {
